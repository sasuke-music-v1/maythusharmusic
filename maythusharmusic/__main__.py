...
async def init():
    ...
    try:
        ...
        pass
    await app.start()
    for mod in app.load_plugins_from("maythusharmusic/plugins"):
    ...
    LOGGER("maythusharmusic.plugins").info("Successfully Imported All Modules ")
    await userbot.start()
    await Yukki.start()
    LOGGER("maythusharmusic").info("Assistant Started Sucessfully")
    try:
    ...
    LOGGER("maythusharmusic").info("maythusharmusic Started Successfully")
    await idle()
    await app.stop()
    await userbot.stop()
...
if __name__ == "__main__":
    app.run(init())
    LOGGER("maythusharmusic").info("Stopping maythusharmusic! GoodBye")
